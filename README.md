社区版不好用，器件要找半天，字段又烧token又耗费命中率；而且后台会直接挂起不能handoff；原版还需要手动确认。。。一点也不方便。原件布局又容易引脚粘连，Net也只是重叠不连接。简单讲，这个版本解决了这些问题。且脱离了vscode的需要，直接使用MCP和插件交互，并提供了丰富的warp，确保流口水的小模型也能顺利完成电路设计和布局。
特殊的，我们引入了OTA机制，可以实现插件热更新。
