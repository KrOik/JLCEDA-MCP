...
调用方法：
1. 根据用户需求整理器件关键词；仅对电阻、电容、电感这类带阻值/容值/感值的器件，若包含数值参数则必须带单位符号，例如电阻写 1kΩ，电容写 100nF，电感写 10uH；禁止使用 1k、100n、10u 这类缺少单位的写法。
...
结果处理：
- 若返回 `ok: true` 且包含 `selectedCandidate`，说明用户已最终确认器件型号，后续必须以该器件为准。
- 即使该器件与你的预期、经验或先前计划不一致，也不得再次调用 `component_select` 要求用户重选，也不得自行改选其他候选器件。只有用户明确要求更换器件时，才允许重新选型。
